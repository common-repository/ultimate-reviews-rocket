...
class ultimate_reviews_rocket_Public {
	 ...
	 * @since    1.0.0
	 */
	public function URR_public_enqueue_styles() {
		...
		wp_enqueue_style( 'urr-public', plugin_dir_url( __FILE__ ) . 'assets/css/ultimate-reviews-rocket-public.css', array(), $this->version, 'all' );
		
	}

	/**
	 * Register the font awesome for the wp_head public-facing side of the site.
	 *
	 * @since    1.0.0
	 */
	public function URR_head_wp_enqueue_scripts() {

		wp_enqueue_style( 'URR-font-awesome', plugin_dir_url( __FILE__ ) . 'assets/css/font-awesome.css', array(), $this->version, 'all' );
	 ...
	 * @since    1.0.0
	 */
	public function URR_public_enqueue_scripts() {
		global $post;
	...
	*/	
	public function rateUs() {
		...
		$content .= '<input name="rating_url" value="'.site_url().'" id="rating_set_url" type="hidden" />';
		$content .= '</form>';
		$content .= '</section>';
		$content .= '</div>';
	...
	*/
	public function feedbackThankYou() {
		...
		if(!empty($_POST['urr_user_name']) && !empty($_POST['urr_email'])){
			...
			$urr_user_name 	= sanitize_text_field($_POST['urr_user_name']);
			$urr_phone 		= sanitize_text_field($_POST['urr_phone']);
			$urr_email 		= sanitize_email($_POST['urr_email']);
			$urr_subject 	= preg_replace("/[^A-Za-z0-9 '\-]/", '',stripslashes_deep($_POST['urr_subject']));
			$urr_message 	= sanitize_textarea_field($_POST['urr_message']);
			$urr_timestamp 	= sanitize_text_field($_POST['urr_timestamp']);
			$urr_user_ip 	= sanitize_text_field($_POST['urr_user_ip']);
			$urr_rating_value = sanitize_text_field($_POST['urr_rating_value']);
			($urr_rating_value >= 3)? $review_type = 'Good' : $review_type = 'Bad';
	
			$urr_data = array(
				'urr_username'				=> $urr_user_name,
				'urr_phone_no'				=> $urr_phone,
				'urr_email'					=> $urr_email,
				'urr_subject'   			=> $urr_subject, 		
				'urr_comments'				=> $urr_message,
				'urr_form_submission_time'  => $urr_timestamp,
				'urr_user_ip'  				=> $urr_user_ip,
				'urr_review_type'  			=> $review_type,
				'urr_rating_value'		 	=> $urr_rating_value,
			);
			$urr_format = array('%s','%s','%s','%s','%s','%s','%s');
			$wpdb->insert( $urr_table_name, $urr_data, $urr_format );
			//database entry
			
	    if ( $urr_email != "" && $urr_user_name != "") {
	...
	*/
	public function socialMediaReviews() {
		...
		if(!empty($this->customeKey['facebook']) || !empty($this->customeKey['instagram'])) {
			$content = "<h3>Great!</h3>";
		...
		if(!empty(trim($this->customeKey['instagram']))) {
			$content .='<li><a href="'.esc_url($this->customeKey['instagram']).'" target="_blank" title="Click here to submit your reviews on Instagram" target="_blank"><i class="fa fa-instagram fa-2x"></i></a></li>';
		}
	...
	public function feedback_form( $review_type  ) {
		...
		(isset($_REQUEST['urr_rating_value']))? $urr_rating_value = sanitize_text_field( $_REQUEST['urr_rating_value'] ): $urr_rating_value = null;
		...
		<form  class="urr-bad-ratings-form"  id="urr-social-media-ratings-form" method="post" action='.$action_url.'>
							<div class="urr-fields  urr-Uname-div">
							...
							<div class="urr-fields urr-Email-div">
